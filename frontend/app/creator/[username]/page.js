...
export default function CreatorPage({ params }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`http://localhost:5000/api/creator/${username}/posts`, {
    ...
    // Load follow status (viewer -> creator)
    (async () => {
      try {
        const res = await fetch(
          `http://localhost:5000/api/follow/status/${encodeURIComponent(username)}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        const data = await res.json().catch(() => ({}));
  ...
  const handleToggleFollow = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/follow/${encodeURIComponent(username)}`, {
        method: 'POST', // backend toggles follow/unfollow
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data?.error || 'Toggle failed');

      // flip UI state based on server truth
      setIsFollowing(!!data.following);

      // (optional) let dashboard counters update instantly
      try {
        ...
        document.dispatchEvent(
          new CustomEvent('onlyskins:follow-changed', {
  ...
  const handleUnlock = async (post) => {
    ...
    try {
      ...
      alert('Redirect to checkout failed: ' + (err?.message || err));
    }
  };
                ...
                {!post.expired && post.purchased_at && (
                  <p className="text-xs text-yellow-400 mt-1">
                    ...
                    {Math.max(
                      ...
                      Math.ceil(
                        (7 * 24 * 60 * 60 * 1000 -
                          (Date.now() - new Date(post.purchased_at).getTime())) /
                          (1000 * 60 * 60 * 24)
                      )
                    )}{' '}
                    day(s)
